...
function App() {
  ...
  if (route === "/counter") return <CustomerApp navigate={navigate} counterMode />;
  if (route === "/admin/forgot-password") return <ForgotPassword navigate={navigate} role="admin" />;
  if (route === "/biller/forgot-password") return <ForgotPassword navigate={navigate} role="biller" />;
  if (route.startsWith("/order/")) return <OrderTracking orderId={route.replace("/order/", "")} />;
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[linear-gradient(135deg,#f6dfc6_0%,#f9b8a9_38%,#f5e5ce_68%,#d8dec8_100%)] text-stone-950">
      ...
      <section className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-7 px-4 py-5 sm:px-6 lg:px-8">
        ...
        <CategoryChips categories={categories} active={activeCategory} setActive={setActiveCategory} />
...
function OrderTracking({ orderId }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const data = await api(`/orders/public/${orderId}`);
        if (!mounted) return;
    ...
    es.addEventListener("order:updated", (e) => {
      try {
        ...
        if (order && ( (payload.id && (payload.id === order._id || payload.id === order.id || payload.id === order.orderId)) || (payload.orderId && payload.orderId === order.orderId) )) {
          load();
        }
      } catch (err) {}
    });
        ...
        <div className="glass-card p-6">
          ...
          <div className="mt-8 rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-2xl">
            ...
            <p className="mt-2 text-sm text-stone-600">
              ...
              {order.paymentStatus === 'rejected' && "Your payment is still under pending process. Please visit the counter and talk to the biller for quick verification."}
            </p>
          </div>
            ...
            <div className="mt-4 space-y-3 text-sm text-stone-700">
              {order.items.map((line, idx) => (
                <div key={idx} className="rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3">
                  ...
                  <p className="mt-2 text-sm text-stone-700">{line.quantity} x {rupees(line.unitPrice)} = {rupees(line.lineTotal)}</p>
                </div>
              ))}
            </div>
          </div>
...
function OwnerApp({ navigate }) {
  ...
  useEffect(() => {
    api("/auth/me").then((data) => setOwner(data.user?.role === "admin" ? data.user.email : null)).finally(() => setLoading(false));
  }, []);
...
function BillerApp({ navigate }) {
  ...
  useEffect(() => {
    api("/auth/me").then((data) => setBiller(data.user?.role === "biller" ? data.user.email : null)).finally(() => setLoading(false));
  }, []);
...
function PendingVerification({ orders, onSaved }) {
  ...
  const pending = (orders || []).filter((o) => (o.paymentStatus === "pending_verification" || o.paymentStatus === "payment_issue") );
      ...
      {pending.map((order) => (
        <article key={order._id || order.id} className="rounded-[1.5rem] bg-white p-5 shadow-sm">
          ...
          <div className="mt-3 grid gap-2 text-sm font-semibold text-stone-700">
            {order.items.map((line, index) => <p key={index}>{line.name} - {line.sizeName}{line.serveType ? ` • ${line.serveType}` : ""} - {line.quantity} x {rupees(line.unitPrice)} = {rupees(line.lineTotal)}</p>)}
          </div>
        </article>
      ))}
    </section>
        ...
        <article key={order._id || order.id} className="rounded-[1.5rem] bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <select value={order.status} onChange={(event) => updateOrderStatus(order._id || order.id, event.target.value)} className="rounded-full bg-stone-100 px-4 py-2 text-sm font-black">
                {['new', 'preparing', 'ready', 'completed', 'cancelled'].map((status) => <option key={status} value={status}>{status}</option>)}
              </select>
              <button onClick={() => openPreview(order, 'customer')} className="rounded-full bg-black px-4 py-2 text-sm font-black text-white">Print Customer Bill</button>
              <button onClick={() => openPreview(order, 'kitchen')} className="rounded-full bg-stone-800 px-4 py-2 text-sm font-black text-white">Print Kitchen Copy</button>
              <button onClick={() => openPreview(order, 'both')} className="rounded-full bg-stone-600 px-4 py-2 text-sm font-black text-white">Print Both</button>
            </div>
          </div>
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-[1.5rem] bg-stone-50 p-4 text-sm text-stone-700">
            <label className="flex items-center gap-2 rounded-full border border-stone-200 bg-white px-4 py-2">
              <input type="checkbox" checked={(printConfig[order._id || order.id] || {}).autoPrint || false} onChange={() => setPrintOption(order._id || order.id, 'autoPrint', !((printConfig[order._id || order.id] || {}).autoPrint))} className="h-4 w-4 accent-black" />
              Auto Print
            </label>
            <span className="text-xs uppercase tracking-[0.2em] text-stone-500">Print from biller panel only</span>
          </div>
          {!hideWarnings && order.warnings?.length > 0 && (
...
function CocAdmin({ cocRequests, onSaved }) {
  ...
  return (
    <section className="space-y-3">
      ...
      {cocRequests.map((req) => (
        <article key={req.id} className="rounded-[1.5rem] bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-xl font-black">{req.customerName} • Table {req.tableNumber}</h2>
              <p className="text-sm font-bold text-stone-500">{req.phone} • {rupees(req.total || 0)} • {req.paymentMethod === 'cash' ? 'Cash on Counter' : 'Online'}</p>
            </div>
            <div className="flex items-center gap-2">
              {isStaff ? (
                <button onClick={() => approve(req.id)} className="rounded-full bg-emerald-700 px-4 py-2 text-sm font-black text-white">Approve & Create Order</button>
              ) : (
                <>
          ...
          <div className="mt-3 grid gap-2 text-sm font-semibold text-stone-700">
            {req.items.map((line, index) => (
              <p key={index}>{line.name || line.itemId} - {line.sizeName || line.sizeId}{line.serveType ? ` • ${line.serveType}` : ""} - {line.quantity} x {rupees(line.unitPrice || 0)} = {rupees((line.unitPrice || 0) * line.quantity)}</p>
            ))}
          </div>
        </article>
      ))}
