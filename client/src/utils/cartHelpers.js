...
export function createCartKey(itemId, sizeId, serveType = "", fallbackServeType = "default", addons = {}) {
  const selectedAddonIds = Array.isArray(addons?.selectedAddons)
    ? addons.selectedAddons.map((addon) => String(addon.id || addon.name || "")).filter(Boolean).sort().join(",")
    : "";
  const extraFlag = selectedAddonIds ? `addons:${selectedAddonIds}` : addons && addons.extraCheese ? "extraCheese" : "noExtra";
...
export function addToCart(cart, item, sizeId, quantity = 1, serveType = "", options = {}) {
  ...
  if (existing) {
    return current.map((line) => (line.key === key ? { ...line, quantity: line.quantity + quantity, lineTotal: Math.round(Number(line.unitPrice || 0) * (line.quantity + quantity)) } : line));
  }

  const selectedAddons = Array.isArray(addons?.selectedAddons) ? addons.selectedAddons : [];
  const safePrice = (value) => Math.round(Number(value || 0));
  const extraPrice = selectedAddons.length
    ? selectedAddons.reduce((sum, addon) => sum + safePrice(addon.price), 0)
    : safePrice(addons?.extraCheesePrice);
  const unitPrice = safePrice(size.price) + extraPrice;
  ...
  return (Array.isArray(cart) ? cart : [])
    .map((line) => {
      ...
      return {
        ...line,
        quantity,
        lineTotal: Math.round(Number(line.unitPrice || 0) * quantity)
      };
    })
...
export function calculateTotals(cart) {
  ...
  const total = lines.reduce((sum, item) => sum + Math.round(Number(item.unitPrice || 0) * Number(item.quantity || 0)), 0);
