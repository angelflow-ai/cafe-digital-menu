...
function formatOrderDateTime(createdAt) {
  ...
  try {
    ...
    return `${day} ${month} ${year} GÇó ${time}`;
  } catch (e) {
...
function OrderLineCard({ line, order, highlightDetails = false }) {
  ...
  const serveText = safeLine.serveType ? ` GÇó ${safeLine.serveType}` : "";
    ...
    <div className="flex flex-col gap-2 rounded-2xl border border-slate-200 bg-slate-50/95 px-4 py-3 shadow-sm ring-1 ring-slate-200 md:flex-row md:items-start md:justify-between md:gap-4">
      <div>
        ...
        <p className="mt-1 text-sm font-semibold text-stone-700">
          ...
          {serveText ? <>{sizeLabel ? " GÇó " : ""}<span className={highlightClassName}>{serveText}</span></> : null}
          {(sizeLabel || serveText) ? " GÇó " : ""}
          ...
          {Array.isArray(form.serveOptions) && form.serveOptions.length > 0 ? (
            <div className="mt-3 flex flex-wrap gap-2">
              {form.serveOptions.map((option) => (
                ...
                >
                  {option}
                  <span className="text-stone-400">+ù</span>
                </button>
              ))}
            </div>
          ) : null}
            ...
            <div className="mt-3 space-y-2">
              {form.addons.map((addon) => (
                <div key={addon.id} className="rounded-2xl border border-stone-200 bg-white p-3">
                  ...
                  <div className="mt-2 text-xs font-semibold text-stone-600">Price: Gé¦{addon.price}</div>
                </div>
              ))}
            </div>
          ) : null}
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-4xl flex-col items-center gap-6 p-4 pt-6 pb-12 sm:pt-10 sm:pb-10">
        <div className="upi-modal-container">
          ...
          <div className="upi-order-summary">
            ...
            <p>{data.phone} GÇó Table {data.tableNumber}</p>
            <div style={{ marginTop: "8px" }}>
              {safeItems.map((line, idx) => (
                <p key={idx}>GÇó {formatOrderItemLine(line)}</p>
              ))}
            </div>
  ...
  useEffect(() => {
    ...
    if (prev !== current) {
      ...
      if (current === 'payment issue' || current === 'rejected' || current === 'payment rejected') {
        ...
        // do NOT redirect or clear cart GÇö customer stays on page until issue is resolved
      }
    }
            ...
            <div className="mt-4 space-y-3 text-sm text-stone-700">
              {order.items.map((line, idx) => (
                <div key={idx} className="rounded-3xl border border-slate-200 bg-slate-50 px-4 py-3">
                  ...
                  <p className="mt-1 text-xs text-stone-600">{normalizeVisibleSizeLabel(line)}{line.serveType ? ` GÇó ${line.serveType}` : ""}</p>
      ...
      {modal.show && modal.type === 'payment_issue' && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/20">
          <div className="rounded-2xl bg-white p-6 text-center border border-rose-200 shadow-lg max-w-md mx-4">
            ...
            <p className="mt-3 text-xs text-stone-500">Order ID: {order.orderId} GÇó Table {order.tableNumber ?? order.table ?? '-'}</p>
          </div>
        </div>
      )}
...
function QuickAccessSection({ quickAccessMode, onQuickAccess, showWaterBottle = true, showCigarettes = true }) {
  const buttons = [
    { key: "water", label: "=ƒÆº Water Bottles", tone: "bg-white/65 hover:bg-white/80", visible: showWaterBottle },
    { key: "cigarettes", label: "=ƒÜ¼ Cigarettes", tone: "bg-white/65 hover:bg-white/80", visible: showCigarettes }
        ...
        <div className="detail-card">
          ...
          <div className="detail-image-wrap">
            {showCigaretteFallback ? (
              <div className="grid h-[180px] w-full place-items-center rounded-[28px] bg-transparent text-[3rem] text-rose-700 shadow-sm ring-1 ring-white/20">=ƒÜ¼</div>
            ) : (
        ...
        <div className="detail-card max-h-[calc(100dvh-16px)] w-full overflow-hidden rounded-[30px] md:max-h-[92vh] md:overflow-visible">
          ...
          <div className="detail-body max-h-[calc(100dvh-132px)] overflow-y-auto p-0 pb-0 sm:max-h-[72vh] md:max-h-none md:overflow-visible">
            ...
            <div className="sticky bottom-0 z-10 px-1 pb-1 pt-0 sm:pb-2">
              ...
              <p className="mt-1 text-center text-[10px] font-semibold text-white/90">GÜá Cigarettes are injurious to health.</p>
            </div>
          </div>
        </div>
      </div>
...
function getCartFallbackIcon(line) {
  ...
  if (isCigaretteItem) {
    return <span className="text-[1.35rem] leading-none" aria-hidden="true">=ƒÜ¼</span>;
  }
  ...
  return <span className="text-[1.15rem] leading-none" aria-hidden="true">=ƒì+n+Å</span>;
}
            ...
            return (
              <div key={safeLine.key || `${safeName}-${Math.random()}`} className="flex gap-3 rounded-3xl bg-white/65 p-2.5">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs font-bold text-stone-500">{normalizeVisibleSizeLabel(safeLine)}{safeLine.serveType ? ` GÇó ${safeLine.serveType}` : ""} - {rupees(getBasePrice(safeLine))} each</p>
                  {(() => {
                  ...
                  <article key={item.id} className="rounded-[1.5rem] bg-stone-50 p-3 shadow-sm ring-1 ring-stone-200">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-sm text-stone-600">{categories.find((cat) => cat.id === item.categoryId)?.name || item.categoryId} GÇó {item.subCategoryName || item.subcategoryName || item.subcategory || "-"}</p>
                      </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <h2 className="text-lg font-black sm:text-xl">
                  <span className="sm:hidden">{order.customerName || "Customer"} GÇó Table {order.tableNumber ?? order.table ?? "-"}</span>
                  <span className="hidden sm:inline">{order.customerName}</span>
                </h2>
                <p className="text-sm font-bold leading-5 text-stone-500">
                  <span className="sm:hidden">{order.phone || "-"} GÇó {rupees(order.total)} GÇó {paymentLabel}</span>
                  <span className="hidden sm:inline">Order {order.orderId || order.id || order._id} GÇó Table {order.tableNumber} GÇó {order.phone} GÇó {rupees(order.total)}</span>
                </p>
              </div>
              {sourceLabel && (
        ...
        return (
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="text-sm font-bold text-stone-500">Order {safeOrder.orderId || safeOrder.id || safeOrder._id || "-"} GÇó Table {safeOrder.tableNumber ?? safeOrder.table ?? "-"} GÇó {safeOrder.phone || "-"} GÇó {rupees(safeOrder.total || 0)}</p>
            </div>
    ...
    <section className="space-y-2.5">
      ...
      {(cocRequests || []).map((req) => {
        ...
        return (
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-xl font-black">{safeRequest.customerName || "Customer"} GÇó Table {safeRequest.tableNumber ?? safeRequest.table ?? "-"}</h2>
              <p className="text-sm font-bold text-stone-500">{safeRequest.phone || "-"} GÇó {rupees(safeRequest.total || 0)} GÇó {safeRequest.paymentMethod === 'cash' ? 'Cash on Counter' : 'Online'}</p>
            </div>
            {sourceLabel && (
              <span className="rounded-full bg-stone-100 px-2 py-1 text-[11px] font-black uppercase tracking-[0.04em] text-stone-700">
                {sourceLabel}
                ...
                <article key={item.id} className="rounded-[1.25rem] border border-stone-200 bg-stone-50 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-stone-600">{item.quantity} {item.unit} GÇó {rupees(item.purchasePrice || 0)}</p>
                    </div>
                    <span className={`rounded-full px-3 py-1 text-xs font-black ${statusClass}`}>{status}</span>
                  </div>
                  <p className="mt-2 text-sm text-stone-500">Min stock: {item.minStock} {item.unit} GÇó Supplier: {item.supplier || "-"}</p>
                  {selectedOutletFilter !== "all" && (
                    <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      ...
                      {transaction.purchasePrice != null && (
                        <p className="mt-1 text-xs font-semibold text-stone-700">Purchase Price: Gé¦{Number(transaction.purchasePrice).toFixed(2)}</p>
                      )}
                      <p className="mt-1 text-xs text-stone-500">{formattedDate} GÇó {formattedTime}</p>
                      {transaction.note && (
          ...
          {(ingredients.length === 0) ? <p>No ingredients configured.</p> : ingredients.map((ingredient, index) => {
            ...
            return (
              <div key={index} className="rounded-3xl bg-stone-50 p-3">
                ...
                <p>{ingredient.amount} {ingredient.unit} {ingredient.serveType ? `GÇó ${ingredient.serveType}` : ""}</p>
              </div>
            );
          })}
        </div>
  ...
  return (
    <section className="space-y-5">
      ...
      {lowMaterials.length > 0 ? (
        ...
        <p className="rounded-[1.5rem] bg-white p-5 text-sm font-bold text-stone-500">All inventory levels are healthy. G£ô</p>
      )}
    </section>
  );
}
...
function SalesAnalytics({ orders = [] }) {
  ...
  const { salesByDate, totalSales, totalOrders, dates, maxSales, dateRangeLabel } = useMemo(() => {
    ...
    const dateRangeLabel = `${range.start.toLocaleDateString("en-IN")} GÇô ${range.end.toLocaleDateString("en-IN")}`;
      ...
      <div className="mt-6 rounded-[1.5rem] bg-stone-50 p-4">
        <div className="mb-6 grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-3">
          ...
          <div>
            ...
            <p className="mt-2 text-2xl font-black text-black">{totalOrders > 0 ? rupees(totalSales / totalOrders) : "Gé¦0"}</p>
          </div>
        </div>
        ...
        {dates.length > 0 ? (
          ...
          <div className="py-16 text-center">
            <p className="text-lg text-stone-400">=ƒôè No sales data</p>
    ...
    <section className="space-y-5">
      <div className="rounded-[1.5rem] bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 rounded-[1.5rem] bg-blue-50 p-5">
          <p className="text-sm font-bold text-blue-700">Gä¦n+Å How Profit is Calculated</p>
          <ul className="mt-3 space-y-2 text-sm text-blue-900">
            <li>G£ô Includes completed QR (Online Payment) and COC (Cash-On-Counter) orders in the selected date range</li>
            <li>G£ô Inventory cost is calculated using recipe mappings (how much raw material each item uses)</li>
            <li>G£ô Raw material costs are based on their unit cost stored in inventory</li>
            <li>G£ô Missing recipe mappings are treated as 0 cost (not included)</li>
            <li>G£ù Cancelled, payment-rejected, or unpaid orders are excluded</li>
          </ul>
        </div>
      </div>
    </section>
            ...
            return (
              <div key={outlet.id} className={`rounded-[1.5rem] p-6 border-2 transition ${isTopOutlet ? "border-emerald-500/30 bg-emerald-50/20 shadow-sm" : "border-stone-200 bg-stone-50/30"}`}>
                <div className="flex items-center justify-between mb-4">
                  ...
                  {isTopOutlet && (
                    <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700 uppercase tracking-wider animate-pulse">=ƒÅå Top Performer</span>
                  )}
                </div>
                <div className="space-y-4">
              ...
              return (
                <tr key={metric.key} className="border-b last:border-0 hover:bg-stone-50/50 transition">
                  ...
                  <td className={`font-black ${winnerIdx === 0 ? "text-emerald-600 font-extrabold" : "text-stone-900"}`}>
                    {metric.format(val0)}
                    {winnerIdx === 0 && <span className="ml-1.5 text-xs text-emerald-600">Gû¦ Higher</span>}
                  </td>
                  <td className={`font-black ${winnerIdx === 1 ? "text-emerald-600 font-extrabold" : "text-stone-900"}`}>
                    {metric.format(val1)}
                    {winnerIdx === 1 && <span className="ml-1.5 text-xs text-emerald-600">Gû¦ Higher</span>}
                  </td>
            ...
            <article key={order._id || order.id} className="rounded-[1.5rem] bg-white p-5 shadow-sm">
              <div className="flex flex-nowrap items-start justify-between gap-2 sm:gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-stone-500">Table {order.tableNumber || order.tableNo || order.table || "-"} GÇó {order.phone}</p>
                </div>
      ...
      <div className="rounded-[1.5rem] bg-white p-5 shadow-sm">
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => setWaterBottleModalOpen(true)} className="rounded-full bg-sky-50 px-3 py-2 text-sm font-black text-sky-700 ring-1 ring-sky-200">=ƒÆº Water Bottle</button>
            <button type="button" onClick={() => setCigarettesModalOpen(true)} className="rounded-full bg-rose-50 px-3 py-2 text-sm font-black text-rose-700 ring-1 ring-rose-200">=ƒÜ¼ Cigarettes</button>
          </div>
        </div>
